...
router.get('/id/:id', (req, res) => {
    ...
    if (parseInt(id) < 1 || parseInt(id) > POKEMONS.length || isNaN(parseInt(id))) {
...
router.get('/type/:lang/:type', (req, res) => {
    ...
    const pokemon = POKEMONS.filter((pokemon) => {
        return pokemon.type.map((type) => type.toLowerCase()).includes(TYPES_LIST['english'][indexType].toLowerCase());
    });
