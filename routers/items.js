...
router.get('/types/:type', (req, res) => {
    ...
    const items = ITEMS.filter((item) => item.type.toLowerCase() === type.toLowerCase());
...
router.get('/name/:lang/:name', (req, res) => {
    ...
    const items = ITEMS.filter((item) => item['name'][LANGUAGES_LIST[lang]].toLowerCase().includes(name.toLowerCase()));
