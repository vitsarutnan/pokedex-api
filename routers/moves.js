...
router.get('/category/:lang/:category', (req, res) => {
    ...
    const moves = MOVES.filter((move) =>
        move['category'][LANGUAGES_LIST[lang]].toLowerCase().includes(category.toLowerCase())
    );
...
router.get('/type/:lang/:type', (req, res) => {
    ...
    if (
        ...
        return res
            .status(400)
            .json({ params: { lang: lang.toLowerCase(), type: type.toLowerCase() }, msg: "It's have something wrong" });
    }

    const moves = MOVES.filter((move) => move['type'][LANGUAGES_LIST[lang]].toLowerCase().includes(type.toLowerCase()));
...
router.get('/name/:lang/:name', (req, res) => {
    ...
    const move = MOVES.filter((move) => move['name'][LANGUAGES_LIST[lang]].toLowerCase().includes(name.toLowerCase()));
